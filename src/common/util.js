...
export default{
  setHeader: function (xhr) {
    xhr.setRequestHeader('Authorization', sessionStorage.getItem(AppConfig.APP_NAME));
  },
  formatDate: {
  ...
  formatDict:{
    ...
    getDictList(dictTypeId){
      //添加业务字典值到缓存
      let list = [];
      if( sessionStorage.getItem(AppConfig.APP_NAME+'_dict_'+dictTypeId) == null ){
        if( dictTypeId != '' ) {
          ...
          baseAjax({
            ...
            success: function (data) {
              list = data;
              sessionStorage.setItem(AppConfig.APP_NAME+'_dict_'+dictTypeId, JSON.stringify(list));
            }.bind(this)
          });
        }
      }else{
        list = JSON.parse(sessionStorage.getItem(AppConfig.APP_NAME+'_dict_'+dictTypeId));  
      }
      return list
    },
  ...
  permissionValid:{
    ...
    hasPermissions: function(obj){
      var permissions = sessionStorage.getItem(AppConfig.APP_NAME+"_user_permission");
