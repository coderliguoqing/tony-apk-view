          ...
          <el-table :data="roleList" highlight-current-row v-loading="listLoading" fit style="width: 100%;">
            ...
            <el-table-column label="操作" width="180">
              <template slot-scope="scope">
                <el-button type="warning"  icon="edit" @click="handleEdit(scope.$index, scope.row)">编辑
                </el-button>
                <el-button type="danger"  icon="delete2" @click="handleDel(scope.$index, scope.row)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      handleEdit: function (index, row) {
        ...
        baseAjax({
          url: api.sys_role.role_info + row.id,
          data: para,
          type: 'GET',
          success: function (data) {
            this.editForm = data;
      ...
      handleDel: function (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          let para = {roleId: row.id};
          baseAjax({
            url: api.sys_role.role_delete + row.id,
            data: para,
            type: 'DELETE',
            dataType: 'text',
            success: function (data) {
