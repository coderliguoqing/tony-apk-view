<template>
  <div>
    <div class="wrapper">
      <div class="uploaded-container" v-if="fileObject.status" v-for="(file, index) in fileList" :key="index">
        <!-- 遮罩 -->
        <div class="mask-div" v-if="uploadLoading && file.url.length > 200"></div>
        <!-- 进度条 -->
        <el-progress class="progress" type="circle" :percentage="percentage" v-if="uploadLoading && file.url.length > 200"></el-progress>
        <div class="delete-icon" @click="removeFile(file)"></div>
        <img v-if="file.url.length > 200" class="uploaded-img" :src="file.url" preview="0"/>
        <img v-else class="uploaded-img" :src="filePath + file.url" preview="0"/>
      </div>
      <!-- 上传按钮 -->
      <div :class="ifShow && fileList.length < countLimit ? 'upload-module' : 'upload-module hidden'">
        <div :id="id"  limitSize="1" :ext="ext" class="upload-btn"></div>
    ...
    WebUploader.Uploader.register(
      ...
      },
      {
          ...
          this.wul_uploader.md5File(file).then(
            function(val) {
              ...
              var signParam =
                ...
                timestamp +
                "}";
              var sign = Base64.encode(
                CryptoJS.HmacSHA1(signParam, this.securityKey)
              );
              // 获取断点续传位置
              jQuery.ajax({
                type: "POST",
        ...
        beforeSend: function(block) {
          ...
          this.wul_uploader.md5File(block.blob).then(
            function(chunkMd5) {
              var timestamp = Date.parse(new Date()) / 1000;
              var signParam =
                "{chunk=" +
      ...
      .delete-icon {
        ...
        &:hover {
          ...
          &::before {
            ...
            right: 0px;
            left: 2px;
            background: url(../../../assets/close_active@2x.png) no-repeat;
            background-size: 20px 20px;
          }
        }
      }
